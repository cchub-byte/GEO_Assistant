...
import { runSamplingPlan, startSamplingPlan } from "@/lib/services/sampling";
...
export async function POST(request: Request) {
  ...
  if (planId) {
    ...
    if (mode === "browser") await startSamplingPlan(planId, mode, safeMaxRuns, options);
    else await runSamplingPlan(planId, mode, safeMaxRuns, options);
  }
  redirect("/sampling");
}
