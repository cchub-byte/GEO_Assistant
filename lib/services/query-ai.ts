    ...
    messages: [
      {
        role: "system",
        content:
          "你是 GEO Query 策略专家。请根据品牌信息与 Query集名称生成用户可能向生成式搜索引擎提出的问题。" +
    ...
    messages: [
      {
        ...
        content:
          ...
          "必须生成 3 组 Query集，每组必须包含 10 条 Query；每组应覆盖不同搜索主题。" +
          ...
          "只返回 JSON，格式为 {\"clusters\":[{\"name\":\"Query集名称\",\"queries\":[{\"queryText\":\"Query 1\",\"intentType\":\"场景模糊\"}]}]}，不要输出解释。"
      },
      {
        role: "user",
        content: JSON.stringify({
...
function normalizeClusterCandidate(item: unknown): GeneratedQueryClusterCandidate | null {
  ...
  const raw = item as { name?: unknown; clusterName?: unknown; queries?: unknown };
  const name = String(raw.name || raw.clusterName || "").trim();
  const rawQueries = Array.isArray(raw.queries) ? raw.queries : [];
  if (!name || rawQueries.length === 0) return null;
